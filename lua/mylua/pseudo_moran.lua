...
function top.init(env)
   ...
   env.smart = Component.Translator(env.engine, "", "script_translator@frequency")
   env.rfixed = moran.Thunk(function()
      return ReverseLookup(env.engine.schema.config:get_string("fixed/dictionary") or "moran_fixed")
end)
   env.fixtranslator = Component.Translator(env.engine, "", "script_translator@translator")
...
function top.func(input, seg, env)
   ...
   if not is_sentence_making or env.quick_code_in_sentence_making then
      ...
      if fixed_res ~= nil then
         if (input_len == 4) then
            ...
            -- if inflexible and env.inject_fixed_words and false then
   ...
   -- if smart_iter ~= nil and (input_len > 4)then
      ...
      local first_cand = smart_iter:peek()
      if first_cand then
         if first_cand.type == "sentence" then
               -- 输出万象整句
               top.wanxiangOutput(input, seg, env)
               local original_cand = smart_iter:next()
               -- 处理后续的释放联想词，如果有
               while smart_iter:peek() and smart_iter:peek().type == "completion" do
                  local completion_cand = smart_iter:next()
                  top.output(env, completion_cand)
               end
               -- 输出原始句子候选
               top.output(env, original_cand)
            end
      end
...
function top.wanxiangOutput(input, seg, env)
   local fixtranslator_iter = moran.make_peekable(moran.query_translation(env.fixtranslator, input, seg, nil))
   local fix_cand = fixtranslator_iter:peek()
   fix_cand.type = 'model'
   top.output(env, fix_cand)
end
