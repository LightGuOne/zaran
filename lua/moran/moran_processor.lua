...
local function steal_auxcode_processor(key_event, env)
    -- ctrl+l, ctrl+o
    -- if not (key_event:ctrl() and (key_event.keycode == 0x6c or key_event.keycode == 0x6f)) then  -- ctrl+l or ctrl+o
    if not (key_event:ctrl() and (key_event.keycode == 0x69 or key_event.keycode == 0x6f)) then    -- ctrl+i or ctrl+o
        return kNoop
    end
...
local SEGMENTATION_PATTERNS = {
    ...
    [8] = {
        {"^[a-z][a-z][ '][a-z][a-z][ '][a-z][a-z][ '][a-z][a-z]$", {3, 3, 2}},  -- 2-2-2-2 => 3-3-2
        {"^[a-z][a-z][a-z][ '][a-z][a-z][a-z][ '][a-z][a-z]$", {3, 2, 3}},      -- 3-3-2 => 3-2-3
        {"^[a-z][a-z][a-z][ '][a-z][a-z][ '][a-z][a-z][a-z]$", {2, 3, 3}},      -- 3-2-3 => 2-3-3
        {"^[a-z][a-z][ '][a-z][a-z][a-z][ '][a-z][a-z][a-z]$", {2, 2, 2, 2}},   -- 2-3-3 => 2-2-2-2
    },
}

local function force_segmentation_processor(key_event, env)
    -- if not (key_event:ctrl() and key_event.keycode == 0x6c) then  -- ctrl+l
    if not (key_event:ctrl() and key_event.keycode == 0x69) then  -- ctrl+i
        return kNoop
    end
...
return {
    init = function(env)
        ...
        env.processors = {
            select_third_candidate_processor,   -- 三选
            semicolon_processor,
