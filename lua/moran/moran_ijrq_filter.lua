...
function Module.func(t_input, env)
   ...
   if input_len == 4 or input_len == 6 or input_len == 8 then
      ...
      local first_text = first_cand:get_genuine().text
      -- 使用正则表达式判断是否为纯英文字母（大小写）
      -- ^ 表示字符串开始，[a-zA-Z] 匹配任何英文字母，+ 表示一个或多个，$ 表示字符串结束
      local is_pure_english = first_text:match("^[a-zA-Z]+$") ~= nil
      -- 只有非纯英文字母的候选词才保存为 last_first_cand
      if not is_pure_english then
         env.last_input = input
         env.last_first_cand = first_text
         env.last_timestamp = env.get_time_ms()
      end
      yield(first_cand)
